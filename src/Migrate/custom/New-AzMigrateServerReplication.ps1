...
"@
            ...
            if ($SiteType -eq $SiteTypes.HyperVSites) {
                $InputObject = Get-AzMigrateHyperVMachine @PSBoundParameters
                $instanceType = $AzStackHCIInstanceTypes.HyperVToAzStackHCI
                
                # Get the site to get project name.
                $null = $PSBoundParameters.Remove('MachineName')
                $siteObject = Get-AzMigrateHyperVSite @PSBoundParameters
            }
            else {
                $InputObject = Get-AzMigrateMachine @PSBoundParameters
            ...
            $policyObj = Az.Migrate\Get-AzMigratePolicy `
            ...
            # Get RunAsAccount
            if ($SiteType -eq $SiteTypes.HyperVSites) {
                $runAsAccounts = Get-AzMigrateHyperVRunAsAccount `
                    -ResourceGroupName $ResourceGroupName `
                    -SiteName $SiteName `
                    -SubscriptionId $SubscriptionId
                ...
                $runAsAccounts = Get-AzMigrateRunAsAccount `
                    -ResourceGroupName $ResourceGroupName `
                    -SiteName $SiteName `
                    -SubscriptionId $SubscriptionId
            ...
            $output = Az.Migrate\New-AzMigrateProtectedItem `
                -Name $MachineName `
                -ResourceGroupName $ResourceGroupName `
                -VaultName $vaultName `
