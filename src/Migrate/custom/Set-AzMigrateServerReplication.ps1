...
function Set-AzMigrateServerReplication {
    ...
    param(
        ...
        [Parameter(ParameterSetName = 'AgentlessVMware_ByID')]
        [Parameter(ParameterSetName = 'AgentlessVMware_ByInputObject')]
        [Microsoft.Azure.PowerShell.Cmdlets.Migrate.Category('Path')]
        [System.String]
    ...
    process {
        ...
        if ($scenario -eq "agentlessVMware") {
            ...
            if ($parameterSet -eq 'AgentlessVMware_ByInputObject') {
            ...
            if ($parameterSet -eq 'AzStackHCI_ByInputObject') {
                $TargetObjectID = $InputObject.Id
            }
            $MachineIdArray = $TargetObjectID.Split("/")
            $ResourceGroupName = $MachineIdArray[4]
