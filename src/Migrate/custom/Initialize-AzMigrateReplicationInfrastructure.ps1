...
function Initialize-AzMigrateReplicationInfrastructure {
    ...
    process {
        if ($Scenario -eq "agentlessVMware") {
            Initialize-AzMigrateReplicationInfrastructureToAzure -ProjectName $ProjectName -ResourceGroupName $ResourceGroupName -Scenario $Scenario -TargetRegion $TargetRegion -CacheStorageAccountId $CacheStorageAccountId -SubscriptionId $SubscriptionId
        }
        else {
            # AzStackHCI scenario
            $null = $PSBoundParameters.Remove('TargetRegion')
            
            Initialize-AzMigrateReplicationInfrastructureToAzStackHCI @PSBoundParameters
        }        
    }
}
